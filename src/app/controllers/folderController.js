...
exports.deleteFolder = async function (req, res) {
  ...
  try {
    ...
    // console.log(selectFolderDetailRows);
...
exports.todayFolder = async function (req, res) {
  ...
  try {
    ...
    PythonShell.run(
      // "/Users/dongjukang/Desktop/matrix_factorization.py",
      "src/app/controllers/recommend/recommend.py",
      options,
...
//   const todayFolderResult = await folderDao.selectTodayFolder(token.userIdx);
//   if (todayFolderResult.isSuccess == false) return res.json(todayFolderResult);

//   const top1FolderResult = await folderDao.selectTopOneFolder(token.userIdx);
//   if (top1FolderResult.isSuccess == false) return res.json(top1FolderResult);

//   if (!todayFolderResult || todayFolderResult.length < 1) {
//     return res.json({
//       isSuccess: false,
//       code: 2000,
//       userIdx: token.userIdx,
//       message: "오늘의 추천 링크달 조회 성공",
//       result: top1FolderResult,
//     });
//   }
//   return res.json({
//     isSuccess: false,
